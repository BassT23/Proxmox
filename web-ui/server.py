...
<body>
  ...
  <script>
    ...
    function isNodeJob(job){return job?.type==='check'&&(/^(?:node[1-9][0-9]*)$/.test(String(job.target))||job.target==='host')||(job?.type==='update'&&(job.target==='host'||String(job.target).startsWith('node-')))}
    async function loadJobs(){try{const d=await api('/api/jobs',{cache:'no-store'}),previous=new Map(jobs.map(job=>[job.unit,job.state]));jobs=sortJobs(Array.isArray(d.jobs)?d.jobs:[]);const nodeJobFinished=jobs.some(job=>isNodeJob(job)&&['completed','failed','interrupted'].includes(job.state)&&['running','pending','starting'].includes(previous.get(job.unit)));renderJobs();reorderJobDom();if(nodeJobFinished)await loadStatus();clearTimeout(pollTimer);pollTimer=setTimeout(loadJobs,jobs.some(j=>j.state==='running')?2000:10000);render(currentStatus)}catch(e){clearTimeout(pollTimer);pollTimer=setTimeout(loadJobs,10000)}}
