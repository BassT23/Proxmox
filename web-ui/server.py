...
<body>
  ...
  <script>
    ...
    const osDetailName=t=>{const raw=text(t.os,'Unknown'),version=text(t.os_version,'');if(!version||raw==='Unknown')return raw;if(/freebsd/i.test(version)&&/pfsense/i.test(raw))return `${raw} / ${version}`;return /\d/.test(raw)?raw:`${raw} ${version}`};
