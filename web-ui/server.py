...
(() => {
  ...
  const formatScheduleTime=value=>{if(!value)return'—';const date=new Date(value);if(Number.isNaN(date.getTime()))return'—';const parts=new Intl.DateTimeFormat('en-US',{month:'short',day:'numeric',year:'numeric',hour:'numeric',minute:'2-digit',hour12:true}).formatToParts(date);const part=type=>parts.find(item=>item.type===type)?.value||'';return`${part('month')} ${part('day')}, ${part('year')} · ${part('hour')}:${part('minute')} ${part('dayPeriod')}`};
...
PAGE = PAGE.replace('</head>', '<style>.version-dialog{width:min(720px,calc(100vw - 32px))}.version-dialog-header{display:flex;align-items:center;justify-content:space-between;gap:16px}.version-dialog-brand{display:flex;align-items:center;gap:12px;min-width:0}.version-dialog-brand img{display:block;width:56px;height:56px;flex:0 0 56px;object-fit:contain}.version-dialog-brand h3{margin:0;min-width:0}.version-dialog-footer{margin-top:16px;padding-top:10px;border-top:1px solid var(--line);text-align:center;font-size:.72rem}.version-dialog-footer a{color:#79bde8;text-decoration:none}.version-dialog-footer a:hover,.version-dialog-footer a:focus-visible{color:#fff;text-decoration:underline}@media(max-width:520px){.version-dialog{width:min(100vw - 20px,720px)}.version-dialog-header{align-items:flex-start;gap:10px}.version-dialog-brand{align-items:flex-start;gap:9px}.version-dialog-brand img{width:48px;height:48px;flex-basis:48px}.version-dialog-brand h3{font-size:1rem}}</style></head>', 1)
