...
<body>
  ...
  <script>
    ...
    const scheduleInteractiveReconnect=state=>{if(!state||state.reconnectScheduled||state.mode==='final'||state.finalizing)return;const unit=state.unit,job=jobs.find(item=>item.unit===unit);if(!job||job.state!=='running')return;state.reconnectScheduled=true;terminalStatus('Reconnecting…');setTimeout(async()=>{if(interactiveTerminal!==state||jobs.find(item=>item.unit===unit)?.state!=='running'){return}const oldAttachment=state.attachment_id;state.finalizing=true;state.inputClosed=true;state.eventSource?.close();state.terminal?.dispose();state.resizeObserver?.disconnect();state.eventSource=null;interactiveTerminal=null;interactiveAttached.delete(unit);interactiveAttachments.delete(unit);try{if(oldAttachment)await api(`/api/jobs/${encodeURIComponent(unit)}/detach`,{method:'POST',body:JSON.stringify({attachment_id:oldAttachment})});await openInteractiveTerminal(unit)}catch(error){terminalStatus('Stream unavailable',true);terminalMessage(error.message||'Live connection unavailable; the job is still running.',true)}finally{if(interactiveTerminal?.unit===unit)interactiveTerminal.reconnectScheduled=false}},1000)};
