...
CONFIG_STRING_KEYS = {
    "ONLY_UPDATE_CHECK", "EXCLUDE_UPDATE_CHECK", "BACKUP_MODE", "BACKUP_STORAGE",
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } .visually-hidden { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0 } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    .auth-loading { text-align:center; color:var(--muted) } .auth-loading .login-branding { margin-bottom:8px }
    ...
    .config-fields { grid-template-columns:repeat(2,minmax(0,1fr)); gap:12px 18px } .config-field { grid-template-columns:minmax(0,1fr) auto; gap:7px 8px } .config-field input[type=checkbox] { grid-column:1; grid-row:1; justify-self:start } .config-field input[type=text],.config-field input[type=number] { grid-column:1 / -1; grid-row:2; width:100% } .config-field .field-label { grid-column:1 / -1; grid-row:1; min-width:0; padding-left:0 } .config-field input[type=checkbox] + .field-label { grid-column:2; justify-self:start } .config-field .field-unit { grid-column:1 / -1; grid-row:3; margin-top:-3px } .config-field.is-dependent { padding-left:18px; border-left:2px solid #94a3b82e } .config-field.is-dependent::before { content:none } .config-field.is-dependent:has(input:disabled) { opacity:.48 } .settings-group .settings-subtitle { color:var(--muted); font-size:.7rem; margin:13px 0 7px; letter-spacing:.04em; text-transform:uppercase }
    ...
    .settings-group { padding-top:10px; margin-top:10px } .settings-group h3 { font-size:.82rem } .settings-group p { margin:3px 0 7px; font-size:.68rem } .config-fields { gap:4px 16px } .config-field.boolean-field { display:flex; align-items:center; gap:8px; min-height:28px; padding:2px 0; border:0; border-radius:0; background:transparent } .config-field.boolean-field input[type=checkbox] { flex:0 0 auto; width:16px; height:16px; margin:0; accent-color:var(--accent) } .config-field.boolean-field .field-label { padding:0; min-width:0; cursor:pointer } .config-field.is-dependent { margin-left:18px; padding-left:10px; border-left:1px solid #94a3b82e } .config-field.is-dependent:has(input:disabled) { opacity:.42 } .config-field:not(.boolean-field) { display:grid; grid-template-columns:minmax(120px,.8fr) minmax(0,1.2fr); align-items:center; gap:5px 10px } .config-field:not(.boolean-field) .field-label { grid-column:1; grid-row:1; padding:0 } .config-field:not(.boolean-field) input[type=text],.config-field:not(.boolean-field) input[type=number] { grid-column:2; grid-row:1; min-width:0; width:100%; padding:6px 8px } .config-field:not(.boolean-field) .field-unit { grid-column:2; grid-row:2; margin:-3px 0 0; font-size:.66rem } .settings-group .settings-subtitle { display:none } .config-actions { position:sticky; bottom:0; z-index:1; margin-top:13px; padding:10px 0 2px; background:linear-gradient(var(--panel),var(--panel)); }
    @media (max-width:760px) { .config-fields { grid-template-columns:1fr } .config-field:not(.boolean-field) { grid-template-columns:minmax(105px,.75fr) minmax(0,1.25fr) } .config-actions { position:static } }
...
<body>
  ...
  <script>
    ...
    function showLogin(message=''){document.getElementById('auth-loading').classList.remove('open');document.getElementById('dashboard').hidden=true;document.getElementById('login-screen').classList.add('open');document.getElementById('login-message').textContent=message;csrfToken=null}
  ...
  <script>
    ...
    const friendlyJobTarget=target=>String(target||'').replace(/^host:/,'');
  ...
  <script>
    const configBooleanKeys=['CHECK_WITH_HOST','CHECK_WITH_LXC','CHECK_WITH_VM','CHECK_RUNNING_CONTAINER','CHECK_STOPPED_CONTAINER','CHECK_RUNNING_VM','CHECK_STOPPED_VM','CHECK_PAUSED_VM','REBOOT_IF_NEEDED','EXIT_ON_ERROR','SNAPSHOT','BACKUP','BACKUP_LXC_MP','EMAIL_DAILY_CHECK','EMAIL_NO_UPDATES','EMAIL_ONLY_SECURITY','EMAIL_ONLY_ERROR'];
    const configNumberKeys=['LXC_START_DELAY','VM_START_DELAY','KEEP_SNAPSHOTS'];
    const configStringKeys=['ONLY_UPDATE_CHECK','EXCLUDE_UPDATE_CHECK','BACKUP_MODE','BACKUP_STORAGE','EMAIL_USER','EMAIL_SENDER'];
    const configLabels={CHECK_WITH_HOST:'Check host',CHECK_WITH_LXC:'Check LXC',CHECK_WITH_VM:'Check VM',CHECK_RUNNING_CONTAINER:'Check running containers',CHECK_STOPPED_CONTAINER:'Check stopped containers',CHECK_RUNNING_VM:'Check running VMs',CHECK_STOPPED_VM:'Check stopped VMs',CHECK_PAUSED_VM:'Check paused VMs',REBOOT_IF_NEEDED:'Reboot if needed',EXIT_ON_ERROR:'Continue after errors',SNAPSHOT:'Create snapshots',KEEP_SNAPSHOTS:'Snapshots to keep',BACKUP:'Create backups',BACKUP_LXC_MP:'Backup LXC mount points',BACKUP_MODE:'Backup mode',BACKUP_STORAGE:'Backup storage',EMAIL_DAILY_CHECK:'Daily email check',EMAIL_NO_UPDATES:'Email when no updates',EMAIL_ONLY_SECURITY:'Email security updates only',EMAIL_ONLY_ERROR:'Email errors only',LXC_START_DELAY:'LXC start delay',VM_START_DELAY:'VM start delay',ONLY_UPDATE_CHECK:'Only update-check filter',EXCLUDE_UPDATE_CHECK:'Exclude update-check filter',EMAIL_USER:'Email recipient',EMAIL_SENDER:'Email sender'};
    const configGroups=[
      {title:'Host',hint:'Host checks are controlled here. Guest settings below do not change host checks.',keys:['CHECK_WITH_HOST']},
      {title:'Containers / LXC',hint:'Choose whether LXC guests and which lifecycle states are included.',keys:['CHECK_WITH_LXC','CHECK_RUNNING_CONTAINER','CHECK_STOPPED_CONTAINER','LXC_START_DELAY','BACKUP_LXC_MP'],dependencies:{CHECK_RUNNING_CONTAINER:'CHECK_WITH_LXC',CHECK_STOPPED_CONTAINER:'CHECK_WITH_LXC',LXC_START_DELAY:'CHECK_WITH_LXC',BACKUP_LXC_MP:'CHECK_WITH_LXC'}},
      {title:'Virtual Machines',hint:'Choose whether VMs and which lifecycle states are included.',keys:['CHECK_WITH_VM','CHECK_RUNNING_VM','CHECK_STOPPED_VM','CHECK_PAUSED_VM','VM_START_DELAY'],dependencies:{CHECK_RUNNING_VM:'CHECK_WITH_VM',CHECK_STOPPED_VM:'CHECK_WITH_VM',CHECK_PAUSED_VM:'CHECK_WITH_VM',VM_START_DELAY:'CHECK_WITH_VM'}},
      {title:'Target filters',hint:'ONLY has priority; EXCLUDE is ignored while ONLY is set.',keys:['ONLY_UPDATE_CHECK','EXCLUDE_UPDATE_CHECK']},
      ...
      {title:'Backup & safety',hint:'Configured protection is evaluated before guest updates.',keys:['SNAPSHOT','KEEP_SNAPSHOTS','BACKUP','BACKUP_MODE','BACKUP_STORAGE'],dependencies:{KEEP_SNAPSHOTS:'SNAPSHOT',BACKUP_MODE:'BACKUP',BACKUP_STORAGE:'BACKUP'}},
    ...
    function updateConfigDependencies(form){for(const input of form.querySelectorAll('[data-parent]')){const parent=form.querySelector(`[data-key="${input.dataset.parent}"]`);const enabled=!parent||parent.checked;input.disabled=!enabled;input.closest('.config-field')?.classList.toggle('is-dependent',Boolean(input.dataset.parent))}}
    function buildConfigForm(values){const form=document.getElementById('config-form');form.innerHTML='';for(const groupData of configGroups){const group=document.createElement('section');group.className='settings-group';group.innerHTML=`<h3>${groupData.title}</h3><p>${groupData.hint}</p>`;const fields=document.createElement('div');fields.className='config-fields';for(const key of groupData.keys){const label=document.createElement('label');label.className=`config-field${configBooleanKeys.includes(key)?' boolean-field':''}`;const caption=document.createElement('span');caption.className='field-label';caption.textContent=configLabels[key]||key;const input=document.createElement('input');input.name=key;input.dataset.key=key;const parent=groupData.dependencies?.[key];if(parent)input.dataset.parent=parent;if(configBooleanKeys.includes(key)){input.type='checkbox';input.checked=values[key]===true;label.append(input,caption)}else{input.type=configNumberKeys.includes(key)?'number':'text';input.value=values[key]??'';if(input.type==='number'){input.min='0';input.max=key==='KEEP_SNAPSHOTS'?'99':'86400'}label.append(caption,input);if(configNumberKeys.includes(key)){const unit=document.createElement('span');unit.className='field-unit';unit.textContent=key==='KEEP_SNAPSHOTS'?'snapshots':'seconds';label.append(unit)}else if(key==='BACKUP_STORAGE'){const unit=document.createElement('span');unit.className='field-unit';unit.textContent='Proxmox storage ID, e.g. pbs';label.append(unit)}}fields.appendChild(label)}group.appendChild(fields);form.appendChild(group)}const actions=document.createElement('div');actions.className='config-actions';actions.innerHTML='<button type="submit" class="primary">Save settings</button><button type="button" id="config-close">Cancel</button>';form.appendChild(actions);const sync=()=>updateConfigDependencies(form);form.querySelectorAll('[data-key]').forEach(input=>input.addEventListener('change',sync));sync();form.onsubmit=async e=>{e.preventDefault();const next={};for(const input of form.querySelectorAll('[data-key]'))next[input.dataset.key]=input.type==='checkbox'?input.checked:input.type==='number'?Number(input.value):input.value;try{const d=await api('/api/config',{method:'POST',body:JSON.stringify({values:next})});buildConfigForm(d.config);setConfigOpen(false);managementMessage('config-message','Configuration saved.')}catch(error){managementMessage('config-message',error.message,true)}};document.getElementById('config-close').onclick=()=>setConfigOpen(false)}
