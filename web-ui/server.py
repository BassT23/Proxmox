...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } .visually-hidden { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0 } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    #settings-page .settings-grid { grid-template-columns:1fr } #settings-page #config-panel { width:100%; min-width:0 } #settings-page #config-form.management-form.open { grid-template-columns:repeat(2,minmax(0,1fr)); align-items:start } #settings-page #config-form .settings-group { min-width:0; margin:0; padding:15px; border:1px solid #94a3b82e; border-radius:12px; background:#0e162b66 } #settings-page #config-form .settings-group-wide,#settings-page #config-form .config-actions { grid-column:1 / -1 }
    @media (max-width:760px) { .config-fields,.settings-columns { grid-template-columns:1fr } #settings-page #config-form.management-form.open { grid-template-columns:1fr } #settings-page #config-form .settings-group-wide,#settings-page #config-form .config-actions { grid-column:auto } .config-field:not(.boolean-field):not(.numeric-field) { grid-template-columns:minmax(105px,.75fr) minmax(0,1.25fr) } .config-field:not(.boolean-field).numeric-field { grid-template-columns:minmax(105px,.75fr) auto auto } .config-actions { position:static } }
...
<body>
  ...
  <main class="app-main" id="dashboard" hidden>
    ...
    <section id="settings-page" class="page-section" hidden><div class="section-title"><div><h2>Settings</h2><span class="hint">Configure the existing Ultimate Updater behavior.</span></div></div><section class="management-grid settings-grid"><section class="management-panel" id="config-panel"><div class="section-title"><div><h2>Configuration</h2><span class="hint">Known settings only · update.conf remains the source of truth</span></div></div><form id="config-form" class="management-form open"></form><div id="config-message" class="management-message"></div></section></section></section>
