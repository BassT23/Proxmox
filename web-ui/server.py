...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } .visually-hidden { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0 } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    .job { display:grid; grid-template-columns:1.6fr 1fr .9fr auto auto; gap:10px; align-items:center; padding:11px 0; border-bottom:1px solid var(--line); font-size:.82rem } .job:last-child { border-bottom:0 } .job code { overflow-wrap:anywhere } .job small,.job-meta { color:var(--muted) } .job-meta { grid-column:1 / -1; font-size:.7rem } .job-download { color:var(--muted); font-size:.72rem; white-space:nowrap } .log { max-height:220px; overflow:auto; white-space:pre-wrap; background:#00000045; border-radius:8px; padding:12px; margin-top:12px; color:#cbd5e1; font: .75rem ui-monospace,monospace } .log-latest { display:block; margin-top:6px; padding:5px 9px; font-size:.7rem }
    ...
    @media (max-width:720px) { main { width:calc(100% - 22px); padding-top:25px } header { display:block } .meta { text-align:left; margin-top:15px } .summary { grid-template-columns:repeat(2,1fr) } .job { grid-template-columns:1fr 1fr } .job button,.job-download { grid-column:span 2; width:100%; text-align:center } }
    ...
    .internal-ssh-add { margin-top:9px; padding:6px 9px; font-size:.72rem; color:var(--muted) } .internal-ssh-add:hover { color:var(--text) } .internal-ssh-view { max-width:900px; margin:18px auto 0 } .internal-ssh-retry { margin-left:7px; padding:5px 8px; font-size:.72rem } .internal-ssh-target-summary { display:grid; gap:4px; padding:8px 10px; border:1px solid var(--line); border-radius:8px; background:#0b122433 } .internal-ssh-target-summary strong { font-size:.7rem; color:var(--muted); font-weight:600 }
...
<body>
  ...
  <main class="app-main" id="dashboard" hidden>
    ...
    <section class="management-grid">
      ...
      <section class="management-panel" id="internal-ssh-card"><div class="section-title"><div><h2>Internal SSH Connections</h2><span class="hint">Manage SSH access for nodes and internal guests.</span></div><span class="help-control"><button class="help-trigger" type="button" aria-label="About Internal SSH Connections" aria-expanded="false" aria-controls="internal-ssh-help">?</button><span id="internal-ssh-help" class="help-popover" role="tooltip"><p>Cluster nodes are detected automatically. Add an SSH connection only when a VM or LXC requires direct SSH access.</p><p>External systems are managed separately under External Targets.</p></span></span><button id="internal-ssh-open" type="button">Open SSH settings</button></div></section>
  ...
  <script>
    ...
    const helpContent={
      'Target filters':[
        'Only limits the check or update to targets with this tag. Leave it empty to include all matching targets.',
        'Exclude removes targets with this tag. If Only is empty, all matching targets are included except excluded ones.',
        'Opening or changing the preview does not contact any target.'
      ]
    };
        ...
        if len(parts) == 4 and parts[:2] == ["api", "jobs"] and parts[3] == "download":
            ...
            try:
                ...
                if not job:
                    raise ValueError
                if job.get("remote"):
                    result = self.run_command([str(self.server.job_runner), "remote-log-full", unit], timeout=60)
                else:
                    result = self.run_command(["journalctl", "-u", unit, "--no-pager", "-o", "cat"], timeout=60)
