...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } .visually-hidden { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0 } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    .interactive-terminal-panel { position:fixed; inset:0; z-index:30; display:grid; place-items:center; padding:clamp(12px,3vw,34px); background:#030712cc; } .interactive-terminal-panel[hidden] { display:none } .interactive-terminal-dialog { width:min(1180px,100%); height:min(90vh,900px); max-height:100%; display:flex; flex-direction:column; padding:20px; border:1px solid #159cf055; border-radius:16px; background:linear-gradient(145deg,#07182bf5,#061323f5); box-shadow:0 24px 90px #000c; } .interactive-terminal { flex:1 1 auto; min-height:0; overflow:hidden; padding:10px; border:1px solid #159cf055; border-radius:9px; background:#050914; } .interactive-terminal .xterm { width:100%; height:100%; } .interactive-terminal-actions { display:flex; align-items:center; justify-content:flex-end; gap:8px; flex-wrap:wrap; margin-top:12px } .interactive-terminal .xterm-viewport { overflow-y:auto !important; }
