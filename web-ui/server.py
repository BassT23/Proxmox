...
<body>
  ...
  <main class="app-main" id="dashboard" hidden>
    ...
    <section class="management-grid">
      <section class="management-panel" id="settings-entry"><div class="section-title"><div><h2>Configuration</h2><span class="hint">Manage Ultimate Updater settings</span></div><a class="button primary" href="/settings">Open settings</a></div></section>
    ...
    <section id="settings-page" class="page-section" hidden><div class="section-title"><div><h2>Settings</h2><span class="hint">Configure the existing Ultimate Updater behavior.</span></div></div><section class="management-grid settings-grid"><section class="management-panel" id="config-panel"><div class="section-title"><div><h2>Configuration</h2><span class="hint">Known settings only · update.conf remains the source of truth</span></div><button id="config-open" type="button">Open editor</button></div><form id="config-form" class="management-form"></form><div id="config-message" class="management-message"></div></section></section></section>
  ...
  <script>
    ...
    function applyPageRoute(push=false){let page=location.pathname==='/settings'?'settings':location.pathname==='/scheduler'?'scheduler':'overview';if(push)history.pushState({},'',page==='overview'?'/':`/${page}`);const subtitles={overview:'A clear overview of updates across your systems.',settings:'Manage configuration without leaving your authenticated session.',scheduler:'Scheduling will be available in a future release.'};document.querySelectorAll('.page-nav a').forEach(link=>{const active=link.dataset.page===page;link.classList.toggle('active',active);link.setAttribute('aria-current',active?'page':'false')});document.getElementById('page-subtitle').textContent=subtitles[page];document.querySelector('.dashboard-kpis').hidden=page!=='overview';document.getElementById('overview-page').hidden=page!=='overview';document.getElementById('settings-page').hidden=page!=='settings';document.getElementById('scheduler-page').hidden=page!=='scheduler';if(page==='settings')loadConfig()}
    document.addEventListener('click',event=>{const link=event.target.closest('.page-nav a');if(!link)return;event.preventDefault();applyPageRoute(true)});window.addEventListener('popstate',()=>applyPageRoute());
  ...
  <script>
    ...
    async function bootstrap(){try{await ensureSession();await Promise.all([loadStatus(),loadJobs(),loadTargets()]);showDashboard();applyPageRoute();scheduleUpdaterVersionCheck()}catch(error){if(csrfToken)notice(error.message,true)}}
