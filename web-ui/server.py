...
<body>
  ...
  <script>
    ...
    async function loadTargetSelection(){try{const data=await api('/api/target-selection');targetSelection={...(data.selection||targetSelection),enabled:data.enabled===true};targetSelectionConfirmed=copyTargetSelection(targetSelection);updateTargetSelectionIndicator();paintTargetSelection()}catch(error){managementMessage('config-message',error.message,true)}}
