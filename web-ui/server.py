...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } .visually-hidden { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0 } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    #settings-page .settings-grid { grid-template-columns:1fr } #settings-page .settings-config-area { min-width:0; margin-top:18px } #settings-page .settings-config-area > .section-title { margin-bottom:16px } #settings-page #config-form.management-form.open { grid-template-columns:repeat(2,minmax(0,1fr)); align-items:start } #settings-page #config-form .settings-group { min-width:0; margin:0; padding:15px; border:1px solid #94a3b82e; border-radius:12px; background:#0e162b66 } #settings-page #config-form .settings-group-wide,#settings-page #config-form .config-actions { grid-column:1 / -1 } #settings-page .connection-management-grid { grid-template-columns:repeat(2,minmax(0,1fr)); margin-top:18px } #settings-page .connection-management-grid > .internal-ssh-view { grid-column:1 / -1; margin-top:0 }
