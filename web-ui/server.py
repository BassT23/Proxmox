...
<body>
  ...
  <script>
    ...
    function createLogLatest(node){let latest=node.parentElement?.querySelector('.log-latest');if(latest)return latest;latest=document.createElement('button');latest.type='button';latest.className='log-latest';latest.textContent='Jump to latest';latest.hidden=true;latest.addEventListener('click',()=>{logAutoFollow=true;node.scrollTop=node.scrollHeight;logScrollTop=node.scrollTop;latest.hidden=true});node.insertAdjacentElement('afterend',latest);return latest}
