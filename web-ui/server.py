...
<body>
  ...
  <script>
    ...
    const helpContent={
      'Target filters':[
        'The check and update previews show the targets that would be included in the respective run based on the current inventory and ONLY/EXCLUDE filters.',
        'Opening or changing the preview does not contact any target.',
        'ONLY has priority for each run type; EXCLUDE is ignored while that ONLY filter is set.'
      ]
    };
    ...
    function openInternalSsh(key){const [kind,id]=key.split(':'),t=internalSshTargets.find(item=>item.kind===kind&&item.id===id)||internalSshAvailable.find(item=>item.kind===kind&&item.id===id),form=document.getElementById('internal-ssh-form');if(!t)return;form.dataset.mode='edit';form.elements.kind.value=kind;form.elements.id.value=id;form.elements.host.value=t.host||'';form.elements.user.value=t.user||'root';form.elements.port.value=t.port||22;form.elements.identity_file.value=t.identity_file||'';form.elements.enabled.checked=t.enabled!==false;document.getElementById('internal-ssh-title').textContent='Edit SSH settings';const picker=document.getElementById('internal-ssh-target-picker');picker.hidden=true;form.elements.target_choice.required=false;const summary=document.getElementById('internal-ssh-target-summary');summary.hidden=false;summary.querySelector('span').textContent=internalSshTargetLabel(t);document.getElementById('internal-ssh-save').textContent='Save SSH settings';document.getElementById('internal-ssh-remove').hidden=!t.override;document.getElementById('internal-ssh-form-message').textContent='';document.getElementById('internal-ssh-modal').classList.add('open')}
