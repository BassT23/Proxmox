...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } .visually-hidden { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0 } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    .config-actions { position:static; background:transparent; } .section-title h2 .help-control { margin-left:6px; vertical-align:middle; }
    ...
    .internal-ssh-add { margin-top:9px; padding:6px 9px; font-size:.72rem; color:var(--muted) } .internal-ssh-add:hover { color:var(--text) } .internal-ssh-view { max-width:900px; margin:18px auto 0 } .internal-ssh-retry { margin-left:7px; padding:5px 8px; font-size:.72rem } .internal-ssh-target-summary { display:grid; gap:4px; padding:8px 10px; border:1px solid var(--line); border-radius:8px; background:#0b122433 } .internal-ssh-target-summary strong { font-size:.7rem; color:var(--muted); font-weight:600 } .ssh-enabled-field { display:flex; align-items:center; gap:8px; min-height:28px } .ssh-enabled-field input[type=checkbox] { flex:0 0 auto; width:16px; height:16px; margin:0; accent-color:var(--accent) } .ssh-test-feedback { display:block; margin-top:5px; color:var(--muted); font-size:.72rem } .ssh-test-feedback.success { color:var(--good) } .ssh-test-feedback.error { color:var(--bad) }
...
<body>
  ...
  <script>
    ...
    const knownNormalUpdates=t=>Number.isInteger(t?.normal_updates)?t.normal_updates:null;
    const knownSecurityUpdates=t=>Number.isInteger(t?.security_updates)?t.security_updates:null;
    const updateValue=value=>Number.isInteger(value)?String(value):'Unknown';
    const updateSummary=targets=>{const items=targets.filter(Boolean),normal=items.map(knownNormalUpdates),security=items.map(knownSecurityUpdates),total=items.map(knownUpdates),normalKnown=normal.every(Number.isInteger),securityKnown=security.every(Number.isInteger),totalKnown=total.every(Number.isInteger),parts=[];if(normalKnown)parts.push(`${normal.reduce((a,v)=>a+v,0)} normal`);else if(totalKnown)parts.push(`${total.reduce((a,v)=>a+v,0)} updates`);if(securityKnown)parts.push(`${security.reduce((a,v)=>a+v,0)} security`);else if(normalKnown)parts.push('security unknown');return parts.join(' · ')||'Updates unknown'};
  ...
  <script>
    ...
    function targetRow(t){const row=document.createElement('div');row.className='target-row';row.innerHTML=`<div><div class="target-name">${guestIdentity(t)}</div><div class="target-id">${esc(t.type)} · ${esc(t.transport)}</div></div><div class="target-field target-status">${statusTone(t)}</div><div class="target-field"><span class="target-label">Normal</span><strong>${updateValue(knownNormalUpdates(t))}</strong></div><div class="target-field"><span class="target-label">Security</span><strong>${updateValue(knownSecurityUpdates(t))}</strong></div><div class="target-field"><span class="target-label">Reboot</span><strong class="${t.reboot_required===true?'reboot-required':''}">${t.reboot_required===true?'Yes':t.reboot_required===false?'No':'Unknown'}</strong></div><div class="target-field row-os"><span class="target-label">OS</span><strong>${esc(osName(t))}</strong></div><div class="target-field row-last-check"><span class="target-label">Last check</span><strong>${esc(date(t.last_check))}</strong></div><div class="row-actions"><button class="check">Check</button><button class="primary update">${running(t.id)?'Running':'Update'}</button></div>`;row.addEventListener('click',e=>{if(!e.target.closest('button'))renderDetails(t)});row.querySelector('.check').addEventListener('click',e=>{e.stopPropagation();action(`/api/check/${encodeURIComponent(t.id)}`)});const update=row.querySelector('.update');update.disabled=running(t.id)||!TARGET_UPDATEABLE(t);update.addEventListener('click',e=>{e.stopPropagation();action(`/api/update/${encodeURIComponent(t.id)}`,true)});return row}
    ...
    const aggregateField=field=>{const targets=Array.isArray(currentStatus?.targets)?currentStatus.targets:[];return targets.length&&targets.every(t=>Number.isInteger(t?.[field]))?targets.reduce((sum,t)=>sum+t[field],0):null};
    const renderWithSplitSummary=render;
    render=function(data){renderWithSplitSummary(data);const normal=aggregateField('normal_updates'),security=aggregateField('security_updates');set('normal-updates',normal===null?'Unknown':normal);set('security-updates',security===null?'Unknown':security)};
    bootstrap();
  </script>
</main></body></html>"""
