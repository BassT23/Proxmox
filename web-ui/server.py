...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } .visually-hidden { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0 } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    .config-fields { grid-template-columns:repeat(2,minmax(0,1fr)); gap:12px 18px } .config-field { grid-template-columns:minmax(0,1fr) auto; gap:7px 8px } .config-field input[type=checkbox] { grid-column:1; grid-row:1; justify-self:start } .config-field input[type=text],.config-field input[type=number] { grid-column:1 / -1; grid-row:2; width:100% } .config-field .field-label { grid-column:1 / -1; grid-row:1; min-width:0; padding-left:0 } .config-field input[type=checkbox] + .field-label { grid-column:2; justify-self:start } .config-field .field-unit { grid-column:1 / -1; grid-row:3; margin-top:-3px } .config-field.is-dependent { padding-left:18px; border-left:2px solid #94a3b82e } .config-field.is-dependent::before { content:'↳'; color:var(--muted); margin-left:-13px; position:absolute } .config-field.is-dependent:has(input:disabled) { opacity:.48 } .settings-group .settings-subtitle { color:var(--muted); font-size:.7rem; margin:13px 0 7px; letter-spacing:.04em; text-transform:uppercase }
...
<body>
  ...
  <script>
    ...
    const configGroups=[
      ...
      {title:'Target filters',hint:'Only has priority. When Only is set, Exclude is ignored by the current core filter logic.',keys:['ONLY_UPDATE_CHECK','EXCLUDE_UPDATE_CHECK']},
