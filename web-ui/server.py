...
def normalize_os_display(os_name, os_version=None):
    """Return the compact OS label used by the overview without losing raw data."""
    raw = " ".join(str(os_name or "").split()).strip()
    version = " ".join(str(os_version or "").split()).strip()
    raw = re.sub(r"^(?:operating system|system|name)\s*:\s*", "", raw, flags=re.I)
    source = " ".join(value for value in (raw, version) if value)
    lower = source.lower()
    ...
    if "freebsd" in lower or "pfsense" in lower:
        found = numbers(r"\b(\d+)(?:\.(\d+))?")
        if not found:
            return "FreeBSD"
        major, minor = found
        return f"FreeBSD {major}" if not minor or minor == "0" else f"FreeBSD {major}.{minor}"
    if "ubuntu" in lower:
        found = numbers(r"\b(\d+)(?:\.(\d+))?")
        return f"Ubuntu {found[0]}.{found[1]}" if found and found[1] else "Ubuntu"
    if "debian" in lower:
        found = numbers(r"\b(\d+)")
        return f"Debian {found[0]}" if found else "Debian"
...
def normalize_os_detail(os_name, os_version=None):
    """Return a useful detail label without exposing kernel build metadata."""
    raw = " ".join(str(os_name or "").split()).strip()
    version = " ".join(str(os_version or "").split()).strip()
    source = " ".join(value for value in (raw, version) if value)
    lower = source.lower()
    if "freebsd" not in lower and "pfsense" not in lower:
        return raw or "Unknown"

    version_match = re.search(
        r"\b(?:freebsd\s+)?(\d+(?:\.\d+)?)(?:-(current|release)(?:-p\d+)?)?",
        source,
        flags=re.I,
    )
    if version_match:
        freebsd = f"FreeBSD {version_match.group(1)}"
        if version_match.group(2):
            freebsd += f"-{version_match.group(2).upper()}"
    else:
        return "pfSense" if "pfsense" in lower else "FreeBSD"
    return f"pfSense / {freebsd}" if "pfsense" in lower else freebsd
...
DEFAULT_SCHEDULER_FILE = Path("/etc/ultimate-updater/schedules.json")
DEFAULT_SCHEDULER_UNIT_DIR = Path("/etc/systemd/system")
...
def scheduler_unit_name(schedule_id):
    ...
    return f"ultimate-updater-schedule-{schedule_id}"


def scheduler_validate(payload, schedule_id=None):
...
def scheduler_next_run_value(raw):
    """Convert systemd's realtime timer value into a local ISO timestamp.

    systemd versions differ here: some return the realtime value as
    microseconds since the epoch, while current systemd returns a localized
    human-readable timestamp even with ``--value``.  The API should expose
    one stable representation to the browser in both cases.
    """
    ...
    try:
        if value.isdigit():
            ...
            if timestamp <= 0:
                return None
            return datetime.fromtimestamp(timestamp / 1_000_000).astimezone().isoformat()
    except (ValueError, OSError, OverflowError):
        return None

    # Example from systemd 255: ``Tue 2026-08-25 04:00:00 CEST``.
...
def scheduler_load(path):
    ...
    for item in data["schedules"]:
        if not isinstance(item, dict) or not isinstance(item.get("id"), str):
            raise ValueError("Scheduler entry has no valid ID")
        schedule_id = item["id"]
        schedules.append(scheduler_validate(item, schedule_id))
    if len({item["id"] for item in schedules}) != len(schedules):
        raise ValueError("Duplicate scheduler ID")
    return schedules


def scheduler_save(path, schedules):
...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } .visually-hidden { position:absolute; width:1px; height:1px; padding:0; margin:-1px; overflow:hidden; clip:rect(0,0,0,0); white-space:nowrap; border:0 } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    #settings-page:not([hidden]) { display:flex; flex-direction:column } #settings-page .settings-config-intro { order:1; margin:0 } #settings-page .settings-management-title,#settings-page .connection-management-grid { order:2 } #settings-page .settings-grid { order:3; grid-template-columns:1fr } #settings-page .settings-config-area { min-width:0; margin-top:0 } #settings-page #config-form.management-form.open { grid-template-columns:repeat(2,minmax(0,1fr)); align-items:start } #settings-page #config-form .settings-group { min-width:0; margin:0; padding:15px; border:1px solid #94a3b82e; border-radius:12px; background:#0e162b66 } #settings-page #config-form .settings-group-wide,#settings-page #config-form .config-actions { grid-column:1 / -1 } #settings-page .connection-management-grid { grid-template-columns:repeat(2,minmax(0,1fr)); margin-top:0 } #settings-page .connection-management-grid > .internal-ssh-view { grid-column:1 / -1; margin-top:0 } #settings-page .internal-ssh-view .settings-group { margin-top:10px; padding:12px; border:1px solid #94a3b82e; border-radius:10px; background:#0e162b66 } #settings-page .internal-ssh-view .managed-target { padding:7px 0 }
    @media (max-width:760px) { .config-fields,.settings-columns { grid-template-columns:1fr } #settings-page #config-form.management-form.open { grid-template-columns:1fr } #settings-page #config-form .settings-group-wide,#settings-page #config-form .config-actions { grid-column:auto } #settings-page .connection-management-grid { grid-template-columns:1fr } #settings-page .connection-management-grid > .internal-ssh-view { grid-column:auto } .config-field:not(.boolean-field):not(.numeric-field) { grid-template-columns:minmax(105px,.75fr) minmax(0,1.25fr) } .config-field:not(.boolean-field).numeric-field { grid-template-columns:minmax(105px,.75fr) auto auto } .config-actions { position:static } }
    ...
    .page-nav { display:flex; flex-wrap:wrap; gap:7px; width:max-content; max-width:100%; margin:16px 0 18px; padding:7px; border:1px solid var(--line); border-radius:14px; background:#151d34e8; box-shadow:0 12px 32px #00000024 } .page-nav a { flex:1 1 auto; min-width:112px; color:var(--muted); border:1px solid transparent; border-radius:9px; padding:9px 16px; text-align:center; font-size:.78rem; text-decoration:none } .page-nav a:hover,.page-nav a:focus-visible { color:var(--text); border-color:var(--line); outline:2px solid #73a7ff55 } .page-nav a.active { color:var(--text); background:#73a7ff2e; border-color:#73a7ff99; box-shadow:inset 0 0 0 1px #73a7ff22; font-weight:700 } .button { display:inline-flex; align-items:center; justify-content:center; border:1px solid var(--line); border-radius:9px; padding:9px 12px; color:var(--text); background:#ffffff0d; cursor:pointer; font:inherit; font-size:.82rem; text-decoration:none } .button.primary { background:#73a7ff24; border-color:#73a7ff88 }
    .page-section[hidden] { display:none } .scheduler-placeholder { max-width:760px; padding:24px } .scheduler-placeholder h2 { margin-top:0 } .scheduler-placeholder p { color:var(--muted); line-height:1.6 } .scheduler-placeholder ul { color:var(--muted); line-height:1.8; padding-left:20px }
    ...
    .dashboard-header { display:block; margin-bottom:18px; padding:0; background:transparent } .dashboard-header-top { display:grid; grid-template-columns:minmax(0,1fr) auto; align-items:start; gap:20px; padding:20px 22px 18px; border:1px solid var(--line); border-radius:18px; background:#151d34aa; box-shadow:0 18px 50px #00000029 } .dashboard-brand { min-width:0 } .dashboard-brand .brand-lockup { gap:16px } .dashboard-brand .brand-header-art { width:min(370px,70vw) } .dashboard-brand .subtitle { margin:5px 0 0; max-width:650px } .dashboard-meta { align-self:start; max-width:360px; padding-top:10px; color:var(--muted); font-size:.82rem; text-align:right } .dashboard-meta button { margin-left:10px; padding:5px 8px; font-size:.7rem } .dashboard-kpis { margin:16px 0 0; }
    ...
    .overview-actions-card { margin-bottom:18px; padding:18px; border:1px solid var(--line); border-radius:18px; background:#151d34aa; box-shadow:0 18px 50px #00000029 } .overview-actions-card h2 { margin:0; font-size:1rem } .overview-actions-card .dashboard-kpis { margin:14px 0 0 } .global-actions { display:flex; flex-wrap:wrap; align-items:center; justify-content:space-between; gap:14px; margin:14px 0 0; padding:14px 0 0; border:0; border-top:1px solid var(--line); border-radius:0; background:transparent; box-shadow:none } .global-actions-copy { min-width:0 } .global-actions-copy strong { display:block; font-size:.92rem } .global-actions-copy span { display:block; margin-top:4px; color:var(--muted); font-size:.74rem } .global-actions-buttons { display:flex; flex-wrap:wrap; gap:9px } .global-action { min-height:42px; padding:10px 16px; font-weight:750; font-size:.84rem } .global-action.update-all { background:#73a7ff36; border-color:#73a7ffb8 } .global-action.check-all { background:#55d39a18; border-color:#55d39a77 }
    @media (max-width:760px) { .dashboard-header { margin-bottom:14px } .dashboard-header-top { display:block; padding:16px 14px 15px } .dashboard-brand .brand-header-art { width:min(270px,82vw) } .dashboard-meta { max-width:none; padding-top:0; margin-top:8px; text-align:left; font-size:.76rem } .page-nav { width:100%; margin:12px 0 14px } .page-nav a { min-width:0; padding:9px 8px } .dashboard-kpis { margin-top:14px } }
    ...
    .target-row.total-only-row { grid-template-columns:minmax(180px,1.5fr) minmax(120px,.9fr) repeat(2,minmax(72px,.7fr)) minmax(80px,.75fr) minmax(140px,1.2fr) minmax(150px,1fr) minmax(132px,max-content); }
    .target-row.total-only-row:not(.lxc-row) > :nth-child(3) { grid-column:span 2; }
    ...
    @media (max-width:1080px) and (min-width:761px) {
      ...
      .target-row.total-only-row { grid-template-columns:minmax(160px,1.35fr) minmax(110px,.85fr) repeat(2,minmax(68px,.7fr)) minmax(76px,.7fr) minmax(130px,1fr) minmax(145px,1fr) minmax(132px,max-content); }
      .target-row.total-only-row:not(.lxc-row) > :nth-child(3) { grid-column:span 2; }
    }
    ...
    .dashboard-header-top { padding:14px 18px 13px; border-color:#159cf0aa; background:linear-gradient(135deg,#061426e8,#071a32cc 55%,#061023e8); box-shadow:0 0 0 1px #0b5c9c33,0 18px 52px #0009,inset 0 1px #ffffff12; }
    ...
    @media (min-width:901px) {
      ...
      .page-nav.expanded { width:158px; overflow:visible; box-shadow:0 18px 50px #0009,0 0 28px #008cff25; }
      ...
      .page-nav.expanded a { padding-right:10px; }
    }
    @media (max-width:900px) {
...
.nav-scrim { position:fixed; inset:0; z-index:19; border:0; border-radius:0; background:#00000066; cursor:default; }
    ...
    @media (min-width:761px) {
      ...
      .page-nav a::before { flex-basis:34px; width:34px; height:28px; margin:0 5px 0 0; background:transparent; box-shadow:none; }
      .page-nav a:nth-child(2)::before { content:"⌂"; }
      .page-nav a:nth-child(3)::before { content:"▦"; }
      ...
      .page-nav a.active::before { color:#fff; background:transparent; box-shadow:none; }
      .nav-toggle { display:grid; place-items:center; flex:0 0 34px; width:34px; height:30px; margin:0 auto 5px 0; padding:0; border:0; border-radius:8px; color:#65bfff; background:transparent; font-size:1.1rem; line-height:1; }
      .nav-toggle:hover,.nav-toggle:focus-visible { color:#fff; background:#0878c933; outline:2px solid #41baff66; }
    }
    @media (max-width:760px) {
    ...
    @media (min-width:761px) {
      .app-main { display:block; position:relative; grid-template-columns:none; width:min(1540px,100%); padding:28px clamp(18px,4vw,54px) 54px; }
      .dashboard-header { display:block; }
...
<body>
  ...
  <div id="updater-version-modal" class="modal-backdrop" role="dialog" aria-modal="true" aria-labelledby="updater-version-title"><section class="modal version-dialog"><div class="version-dialog-header"><div class="version-dialog-brand"><img src="/assets/ultimate-updater-icon.png" alt="" aria-hidden="true"><h3 id="updater-version-title">Ultimate Updater version</h3></div><button type="button" class="modal-close" id="updater-version-close">Close</button></div><div id="updater-version-content"><p class="hint">Loading version information…</p></div><div class="form-actions"><button type="button" id="updater-version-check">Check again</button><button type="button" class="primary" id="updater-version-update">Update now</button></div><div id="updater-version-message" class="management-message" role="status"></div><footer class="version-dialog-footer"><a href="https://github.com/BassT23/Proxmox" target="_blank" rel="noopener noreferrer">GitHub · BassT23/Proxmox</a></footer></section></div>
  <script>
    ...
    const date=v=>{if(!v)return'Unknown';const d=new Date(v);return Number.isNaN(d.getTime())?String(v):d.toLocaleString()}; const statusLabel=v=>labels[v]||['Unknown','neutral']; const set=(id,v)=>diagnosticTextWrite(document.getElementById(id),v,`set(${id})`,`#${id}`);
    ...
    function setLoginLoading(loading){const form=document.getElementById('login-form'),button=form.querySelector('button[type="submit"]');form.classList.toggle('is-loading',loading);form.dataset.submitting=loading?'true':'false';button.disabled=loading;diagnosticTextWrite(button,loading?'Signing in…':'Sign in','setLoginLoading submit button','button[type="submit"]');form.querySelectorAll('input').forEach(input=>{input.disabled=loading})}
    ...
    const interactiveDiagnosticsSent=new Set();
    const reportInteractiveDiagnostic=report=>{const key=[report.kind,report.context,report.selector,report.message].join('|');if(interactiveDiagnosticsSent.has(key))return;interactiveDiagnosticsSent.add(key);if(!csrfToken)return;const unit=typeof interactiveTerminal==='undefined'?null:interactiveTerminal?.unit;fetch('/api/diagnostics/interactive-dom',{method:'POST',headers:{'Content-Type':'application/json','X-CSRF-Token':csrfToken},body:JSON.stringify({...report,unit,url:location.href})}).catch(()=>{})};
    window.addEventListener('error',event=>{if(!String(event.message||'').includes('textContent'))return;reportInteractiveDiagnostic({kind:'window-error',message:String(event.message),filename:event.filename||'',line:event.lineno||0,column:event.colno||0,stack:event.error?.stack||''})});
    window.addEventListener('unhandledrejection',event=>{const reason=event.reason||{};const message=String(reason.message||reason);if(!message.includes('textContent'))return;reportInteractiveDiagnostic({kind:'unhandled-rejection',message,filename:'',line:0,column:0,stack:reason.stack||''})});
    const diagnosticTextWrite=(element,value,context,selector='')=>{if(!element){const error=new TypeError("Cannot set properties of null (setting 'textContent')");const stack=error.stack||'';reportInteractiveDiagnostic({kind:'textcontent-write',context,selector,message:error.message,filename:'',line:0,column:0,stack});throw error}element.textContent=value;return element};
  ...
  <script>
    ...
    const osOverviewName=t=>text(t.os_display,'Unknown');
    const osDetailName=t=>text(t.os_detail_display,t.os||'Unknown');
    const healthState=t=>{if(t?.health_state)return t.health_state;const status=t?.check_status;if(status==='error'||status==='offline')return'attention';if(!['ok','updates_available'].includes(status))return['not_checked','skipped','stopped','unknown',undefined,null,''].includes(status)?'unknown':'known';if(t?.reachable!==true||!t?.last_check||!Number.isInteger(t?.updates?.available))return'unknown';const split=securitySplitSupported(t);return split&&(!Number.isInteger(t?.normal_updates)||!Number.isInteger(t?.security_updates))?'unknown':'known'};
    ...
    function renderJobsStable(){const n=document.getElementById('jobs');if(!jobs.length){n.hidden=true;openJobLogId=null;return}if(openJobLogId&&!jobs.some(j=>j.unit===openJobLogId))openJobLogId=null;n.hidden=false;let list=n.querySelector('.job-list');if(!list||!n.querySelector('.job-toggle')){n.innerHTML='<div class="section-title"><button class="job-toggle"><span class="chevron" aria-hidden="true"></span><span>Jobs <span class="job-count"></span></span></button><span class="job-summary">Server-side state · safe across browser/device changes</span></div><div class="job-list"></div>';n.querySelector('.job-toggle').addEventListener('click',()=>{jobsExpanded=!jobsExpanded;n.classList.toggle('collapsed',!jobsExpanded)});list=n.querySelector('.job-list')}n.classList.toggle('collapsed',!jobsExpanded);diagnosticTextWrite(n.querySelector('.job-count'),`(${jobs.filter(j=>j.state==='running').length} running, ${jobs.filter(j=>j.state!=='running').length} finished)`,'renderJobsStable job-count','.job-count');const existing=new Map([...list.querySelectorAll('.job')].map(item=>[item.dataset.unit,item]));const seen=new Set();for(const j of jobs){let item=existing.get(j.unit);if(!item){item=document.createElement('div');item.className='job';item.dataset.unit=j.unit;item.innerHTML=`<code></code><span></span><span class="pill"></span><button data-job="${esc(j.unit)}">Show log</button><div class="log" id="log-${esc(j.unit)}" hidden></div>`;const button=item.querySelector('button[data-job]');button.addEventListener('click',async()=>{const unit=button.dataset.job,node=document.getElementById(`log-${unit}`);if(openJobLogId===unit){openJobLogId=null;finalLogLoaded.delete(unit);node.hidden=true;item.querySelector('.log-actions')?.remove();button.textContent='Show log';return}if(openJobLogId&&openJobLogId!==unit){const previous=[...document.querySelectorAll('.job')].find(candidate=>candidate.dataset.unit===openJobLogId);previous?.querySelector('.log-actions')?.remove();const previousLog=previous?.querySelector('.log');if(previousLog)previousLog.hidden=true;const previousButton=previous?.querySelector('button[data-job]');if(previousButton)previousButton.textContent='Show log'}openJobLogId=unit;finalLogLoaded.delete(unit);logAutoFollow=true;logScrollTop=0;node.hidden=false;button.textContent='Hide log';createLogActions(node,unit);const latest=createLogLatest(node,unit);latest.hidden=true;await loadJobLog(unit,node);attachLogScroll(node)});list.appendChild(item)}seen.add(j.unit);diagnosticTextWrite(item.querySelector('code'),j.unit,'renderJobsStable job unit',`#jobs .job[data-unit="${j.unit}"] code`);item.querySelector('code').title=j.unit;diagnosticTextWrite(item.querySelector('span'),j.owner_node?`${friendlyJobTarget(j.target)} · ${j.owner_node}`:friendlyJobTarget(j.target),'renderJobsStable job target',`#jobs .job[data-unit="${j.unit}"] > span`);const state=item.querySelector('.pill');diagnosticTextWrite(state,j.state==='completed_with_warnings'?'Please check':j.state,'renderJobsStable job state',`#jobs .job[data-unit="${j.unit}"] .pill`);state.className=`pill ${j.state==='completed'?'good':j.state==='failed'||j.state==='interrupted'?'bad':'warn'}`;const button=item.querySelector('button[data-job]'),node=item.querySelector('.log'),open=j.unit===openJobLogId;diagnosticTextWrite(button,open?'Hide log':'Show log','renderJobsStable log button',`#jobs .job[data-unit="${j.unit}"] button[data-job]`);node.hidden=!open}for(const [unit,item] of existing){if(!seen.has(unit))item.remove()}if(openJobLogId){const job=jobs.find(j=>j.unit===openJobLogId),node=document.getElementById(`log-${openJobLogId}`);if(job&&node){node.hidden=false;loadJobLog(openJobLogId,node).then(()=>{if(openJobLogId===node.id.slice(4))attachLogScroll(node)})}}}
    ...
    const interactiveAttached=new Set(),interactiveAttachments=new Map(),interactiveQueues=new Map();let interactiveTerminal=null;
    ...
    const openInteractiveTerminal=async unit=>{if(typeof Terminal!=='function'||typeof FitAddon==='undefined'||typeof FitAddon.FitAddon!=='function'){notice('The interactive terminal assets are unavailable.',true);return}if(interactiveTerminal?.unit===unit){interactiveTerminal.terminal.focus();return}if(interactiveTerminal)await disposeInteractiveTerminal(true);let attachment_id=interactiveAttachments.get(unit),panel=null;try{if(!attachment_id){const result=await api(`/api/jobs/${encodeURIComponent(unit)}/attach`,{method:'POST',body:'{}'});attachment_id=result.attachment_id;if(!attachment_id)throw new Error('The interactive attachment is unavailable.');interactiveAttachments.set(unit,attachment_id);interactiveAttached.add(unit)}const container=document.getElementById('interactive-terminal'),job=jobs.find(item=>item.unit===unit);panel=document.getElementById('interactive-terminal-panel');panel.hidden=false;const title=document.getElementById('interactive-terminal-title'),download=document.getElementById('interactive-terminal-download');if(title)title.textContent=job?jobTitle(job):unit;if(download)download.href=`/api/jobs/${encodeURIComponent(unit)}/download`;terminalMessage('');interactiveKeybarState(false);const fontSize=readInteractiveFontSize(),terminal=new Terminal({scrollback:2000,convertEol:false,fontSize});const fitAddon=new FitAddon.FitAddon();terminal.loadAddon(fitAddon);terminal.open(container);interactiveTerminal={unit,attachment_id,terminal,fitAddon,fontSize,eventSource:null,inputBuffer:[],inputTimer:null,inputClosed:false,resizeObserver:null,resizeTimer:null,viewportHandler:null,resizeInFlight:false,resizeAgain:false,lastSize:null};const state=interactiveTerminal;updateInteractiveFontLabel(fontSize);terminal.attachCustomKeyEventHandler(event=>{if(event.type!=='keydown')return true;if(event.ctrlKey&&event.key.toLowerCase()==='c'){event.preventDefault();disposeInteractiveTerminal(true);return false}if(event.ctrlKey&&(event.key.toLowerCase()==='d'||event.key.toLowerCase()==='z')){event.preventDefault();terminalMessage('This control key is disabled for the interactive job.',true);return false}return true});terminal.onData(queueInteractiveInput);fitAddon.fit();await sendInteractiveResize(true);const refit=()=>{if(interactiveTerminal!==state)return;scheduleInteractiveResize()};state.viewportHandler=refit;if(typeof ResizeObserver==='function'){state.resizeObserver=new ResizeObserver(refit);state.resizeObserver.observe(container)}window.addEventListener('orientationchange',refit);window.visualViewport?.addEventListener('resize',refit);terminal.focus();const source=new EventSource(`/api/jobs/${encodeURIComponent(unit)}/stream?attachment_id=${encodeURIComponent(attachment_id)}&from=0`);state.eventSource=source;terminalStatus('Connecting…');source.addEventListener('attached',()=>terminalStatus('Connected'));source.addEventListener('output',event=>{try{terminal.write(terminalBytes(event.data))}catch(error){terminalMessage('Terminal output could not be rendered.',true);terminalStatus('Stream error',true)}});source.addEventListener('truncated',()=>terminalMessage('Earlier terminal output is no longer available. Use “Download full log” for the complete journal.',true));source.addEventListener('heartbeat',()=>{});source.addEventListener('closed',()=>{source.close();if(interactiveTerminal===state){state.inputClosed=true;interactiveKeybarState(true);terminalStatus('Job finished')}});source.onerror=()=>{if(source.readyState===EventSource.CLOSED){terminalStatus('Stream unavailable',true);terminalMessage('The live terminal stream ended. The existing job log remains available.',true)}else terminalStatus('Reconnecting…')};decorateInteractiveJobs()}catch(error){const message=error.message||'The interactive terminal could not be opened.',state=interactiveTerminal;if(state?.unit===unit){state.inputClosed=true;if(state.inputTimer)clearTimeout(state.inputTimer);if(state.resizeTimer)clearTimeout(state.resizeTimer);state.resizeObserver?.disconnect();if(state.viewportHandler){window.visualViewport?.removeEventListener('resize',state.viewportHandler);window.removeEventListener('orientationchange',state.viewportHandler)}state.eventSource?.close();terminalStatus('Stream unavailable',true)}else if(attachment_id){try{await api(`/api/jobs/${encodeURIComponent(unit)}/detach`,{method:'POST',body:JSON.stringify({attachment_id})})}catch(_error){}}if(panel){panel.hidden=false;interactiveKeybarState(true);terminalMessage(message,true);terminalStatus('Stream unavailable',true)}else notice(message,true);interactiveAttached.delete(unit);interactiveAttachments.delete(unit);interactiveQueues.delete(unit)}};
    const openLiveOutput=async unit=>{if(typeof Terminal!=='function'||typeof FitAddon==='undefined'||typeof FitAddon.FitAddon!=='function'){notice('The live output assets are unavailable.',true);return}if(interactiveTerminal?.unit===unit){interactiveTerminal.terminal.focus();return}if(interactiveTerminal)await disposeInteractiveTerminal(true);try{const panel=document.getElementById('interactive-terminal-panel'),container=document.getElementById('interactive-terminal'),job=jobs.find(item=>item.unit===unit);panel.hidden=false;document.getElementById('interactive-terminal-heading').textContent='Live output';document.getElementById('interactive-terminal-title').textContent=job?jobTitle(job):unit;document.getElementById('interactive-terminal-download').href=`/api/jobs/${encodeURIComponent(unit)}/download`;terminalMessage('');interactiveKeybarVisibility(false);interactiveKeybarState(true);document.getElementById('interactive-terminal-help').hidden=true;const fontSize=readInteractiveFontSize(),terminal=new Terminal({scrollback:2000,convertEol:false,fontSize,disableStdin:true});const fitAddon=new FitAddon.FitAddon();terminal.loadAddon(fitAddon);terminal.open(container);interactiveTerminal={unit,mode:'output',attachment_id:null,terminal,fitAddon,fontSize,eventSource:null,inputBuffer:[],inputTimer:null,inputClosed:true,resizeObserver:null,resizeTimer:null,viewportHandler:null,resizeInFlight:false,resizeAgain:false,lastSize:null};const state=interactiveTerminal;updateInteractiveFontLabel(fontSize);fitAddon.fit();const refit=()=>{if(interactiveTerminal===state)scheduleInteractiveResize()};state.viewportHandler=refit;if(typeof ResizeObserver==='function'){state.resizeObserver=new ResizeObserver(refit);state.resizeObserver.observe(container)}window.addEventListener('orientationchange',refit);window.visualViewport?.addEventListener('resize',refit);terminal.focus();const source=new EventSource(`/api/jobs/${encodeURIComponent(unit)}/output-stream`);state.eventSource=source;terminalStatus('Connecting…');source.addEventListener('attached',()=>terminalStatus('Connected'));source.addEventListener('output',event=>{try{terminal.write(terminalBytes(event.data))}catch(error){terminalMessage('Live output could not be rendered.',true);terminalStatus('Stream error',true)}});source.addEventListener('heartbeat',()=>{});source.addEventListener('closed',()=>{source.close();if(interactiveTerminal===state)terminalStatus('Job finished')});source.onerror=()=>{if(source.readyState===EventSource.CLOSED){terminalStatus('Stream unavailable',true);terminalMessage('The live output stream ended. The existing job log remains available.',true)}else terminalStatus('Reconnecting…')};}catch(error){await disposeInteractiveTerminal(false);notice(error.message||'The live output could not be opened.',true)}};
    document.getElementById('interactive-terminal-detach').onclick=()=>disposeInteractiveTerminal(true);
    const decorateInteractiveJob=item=>{const unit=item.dataset.unit,job=jobs.find(candidate=>candidate.unit===unit),available=job?.interactive&&job?.socket_available&&job?.state==='running';let controls=item.querySelector('.interactive-controls');if(!available){if(controls)controls.remove();interactiveAttached.delete(unit);if(!interactiveTerminal||interactiveTerminal.unit!==unit)interactiveAttachments.delete(unit);return}if(!controls){controls=document.createElement('div');controls.className='interactive-controls';controls.innerHTML='<span class="interactive-status"></span><button type="button" data-interactive-terminal>Live terminal</button>';item.appendChild(controls);controls.querySelector('[data-interactive-terminal]').onclick=()=>openInteractiveTerminal(unit)}const attached=interactiveAttached.has(unit),status=controls.querySelector('.interactive-status'),terminalButton=controls.querySelector('[data-interactive-terminal]');diagnosticTextWrite(status,attached?'Interactive terminal attached':'Interactive terminal available','decorateInteractiveJob status','.interactive-status');diagnosticTextWrite(terminalButton,interactiveTerminal?.unit===unit?'Terminal open':'Live terminal','decorateInteractiveJob button','[data-interactive-terminal]');terminalButton.disabled=false};
    const decorateInteractiveJobs=()=>document.querySelectorAll('#jobs .job').forEach(decorateInteractiveJob);
    const decorateAllLiveJobs=()=>document.querySelectorAll('#jobs .job').forEach(item=>{const unit=item.dataset.unit,job=jobs.find(candidate=>candidate.unit===unit),running=job?.state==='running',interactive=job?.interactive===true;let controls=item.querySelector('.interactive-controls');if(!running){if(controls)controls.remove();if(!interactiveTerminal||interactiveTerminal.unit!==unit)interactiveAttachments.delete(unit);return}if(!controls){controls=document.createElement('div');controls.className='interactive-controls';controls.innerHTML='<span class="interactive-status"></span><button type="button" data-live-job></button>';item.appendChild(controls);controls.querySelector('[data-live-job]').onclick=()=>interactive?openInteractiveTerminal(unit):openLiveOutput(unit)}const status=controls.querySelector('.interactive-status'),button=controls.querySelector('[data-live-job]');diagnosticTextWrite(status,interactive?(interactiveAttached.has(unit)?'Interactive terminal attached':'Interactive terminal available'):'Live output available','decorateAllLiveJobs status','.interactive-status');diagnosticTextWrite(button,interactive?(interactiveTerminal?.unit===unit?'Terminal open':'Live terminal'):'Live output','decorateAllLiveJobs button','[data-live-job]');button.disabled=false});
...
PAGE = re.sub(r'<section class="summary dashboard-kpis"><div class="metric"><strong id="total">.*?</section>', '<section class="summary dashboard-kpis"><div class="metric"><div class="metric-top"><span class="overview-summary-icon" data-overview-icon="systems"></span><strong id="total">–</strong></div><span>known systems</span></div><div class="metric"><div class="metric-top"><span class="overview-summary-icon" data-overview-icon="check"></span><strong id="online">–</strong></div><span>reachable</span></div><div class="metric"><div class="metric-top"><span class="overview-summary-icon" data-overview-icon="download"></span><strong id="normal-updates">–</strong></div><span>normal updates</span></div><div class="metric"><div class="metric-top"><span class="overview-summary-icon" data-overview-icon="shield"></span><strong id="security-updates">–</strong></div><span>security updates</span></div><div class="metric"><div class="metric-top"><span class="overview-summary-icon" data-overview-icon="package"></span><strong id="other-updates">–</strong></div><span>other updates</span></div><div class="metric"><div class="metric-top"><span class="overview-summary-icon" data-overview-icon="attention"></span><strong id="attention">–</strong></div><span>needs attention</span></div></section>', PAGE, count=1, flags=re.S)
PAGE = PAGE.replace('    bootstrap();', '''    const statusIcon=kind=>{const paths={check:'<circle cx="12" cy="12" r="8.5"/><path d="m8 12 2.5 2.5L16 9"/>',download:'<path d="M12 4v10m0 0 4-4m-4 4-4-4M5 19h14"/>',shield:'<path d="M12 3 19 6v5c0 4.4-2.9 7.2-7 9-4.1-1.8-7-4.6-7-9V6l7-3Z"/><path d="M12 8v4m0 3h.01"/>',reboot:'<path d="M19 8a8 8 0 0 0-13.8-1.6L4 8"/><path d="M4 4v4h4M5 16a8 8 0 0 0 13.8 1.6L20 16"/><path d="M20 20v-4h-4"/>',offline:'<circle cx="12" cy="12" r="8.5"/><path d="m9 9 6 6m0-6-6 6"/>',attention:'<path d="m12 3 9 16H3L12 3Z"/><path d="M12 9v4m0 3h.01"/>',unknown:'<circle cx="12" cy="12" r="8.5"/><path d="M9.6 9a2.5 2.5 0 1 1 4.1 1.9c-1 .8-1.7 1.2-1.7 2.6m0 2.5h.01"/>',activity:'<path d="M4 12h3l2-5 4 10 2-5h5"/>'};return `<svg class="status-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true" focusable="false">${paths[kind]||paths.unknown}</svg>`};
    const statusIconKind=element=>{const textValue=element.textContent.trim().toLowerCase();if(element.classList.contains('reboot-required-badge'))return'reboot';if(textValue.includes('security'))return'shield';if(textValue.includes('update')||textValue.includes('available'))return'download';if(textValue.includes('healthy')||textValue.includes('completed')||textValue==='ok')return'check';if(textValue.includes('offline'))return'offline';if(textValue.includes('error')||textValue.includes('failed')||textValue.includes('please check')||textValue.includes('attention'))return'attention';if(textValue.includes('running')||textValue.includes('checking'))return'activity';return'unknown'};
    const decorateStatusIcons=root=>{root?.querySelectorAll('.pill,.reboot-required-badge').forEach(element=>{if(!element.querySelector('.status-icon'))element.insertAdjacentHTML('afterbegin',statusIcon(statusIconKind(element)))});};
...
PAGE = PAGE.replace('<section class="summary dashboard-kpis">', '<section class="overview-actions-card"><h2>System overview</h2><section class="summary dashboard-kpis">', 1)
PAGE = PAGE.replace('</section>\n    <section id="systems"', '</section></section>\n    <section id="systems"', 1)
PAGE = PAGE.replace('<div class="section-title"><div><h2>Settings</h2><span class="hint">Configure the existing Ultimate Updater behavior.</span></div></div>', '<div class="settings-config-intro section-title"><div><h2>Configuration</h2><span class="hint">Known settings only · update.conf remains the source of truth</span></div></div>', 1)
PAGE = PAGE.replace('<section class="settings-config-area" id="config-panel"><div class="section-title"><div><h2>Configuration</h2><span class="hint">Known settings only · update.conf remains the source of truth</span></div></div><form', '<section class="settings-config-area" id="config-panel"><form', 1)
...
# markup is injected. Keep the event hook explicit so in-page navigation
# refreshes scheduler data without changing the existing routing model.
if "window.dispatchEvent(new Event('scheduler-page-open'))" not in PAGE:
    PAGE = PAGE.replace("if(page==='settings')loadConfig()}", "if(page==='settings')loadConfig();if(page==='scheduler')window.dispatchEvent(new Event('scheduler-page-open'))}")
PAGE = PAGE.replace('</body></html>', '''<script>
(() => {
...
def derive_health_state(target):
    """Classify whether the projected target has enough current health data."""
    if not isinstance(target, dict):
        return "unknown"
    check_status = target.get("check_status")
    if check_status in {"error", "offline"}:
        return "attention"
    if check_status in {None, "", "unknown", "not_checked", "skipped", "stopped"}:
        return "unknown"
    if check_status not in {"ok", "updates_available"}:
        # Unsupported/other explicit states retain their existing neutral UI.
        return "known"
    if target.get("reachable") is False:
        return "attention"
    if target.get("reachable") is not True or not target.get("last_check"):
        return "unknown"
    updates = target.get("updates")
    if not isinstance(updates, dict) or type(updates.get("available")) is not int:
        return "unknown"
    split_supported = target.get("security_split_supported") is True
    if "security_split_supported" not in target:
        split_supported = (
            target.get("updater") == "apt"
            or "normal_updates" in target
            or "security_updates" in target
        )
    if split_supported and not all(
        type(target.get(key)) is int for key in ("normal_updates", "security_updates")
    ):
        return "unknown"
    return "known"


def annotate_health_state(payload):
    """Add transient health semantics to an API projection, never to status.json."""
    projected = dict(payload)
    projected["targets"] = []
    for target in payload.get("targets", []):
        item = dict(target) if isinstance(target, dict) else target
        if isinstance(item, dict):
            item["health_state"] = derive_health_state(item)
            item["os_display"] = normalize_os_display(item.get("os"), item.get("os_version"))
            item["os_detail_display"] = normalize_os_detail(item.get("os"), item.get("os_version"))
...
def active_inventory_projection(payload, inventory):
    ...
    return annotate_health_state(projected)
...
class StatusHandler(BaseHTTPRequestHandler):
    ...
    def scheduler_systemctl(self, *arguments, timeout=15):
        result = self.run_command(["systemctl", *arguments], timeout=timeout)
        if result.returncode:
            detail = (result.stderr or result.stdout or "").strip().replace("\n", " ")[:300]
            raise RuntimeError(detail or "systemd scheduler operation failed")
        return result
    ...
    def scheduler_write_units(self, schedule):
        ...
        elif (self.server.scheduler_unit_dir / f"{unit}.timer").exists():
            try:
                self.scheduler_systemctl("disable", "--now", f"{unit}.timer")
            except RuntimeError:
                pass
        return unit

    def scheduler_remove_unit(self, schedule_id):
        unit = scheduler_unit_name(schedule_id)
        try:
            self.scheduler_systemctl("disable", "--now", f"{unit}.timer")
        except RuntimeError:
            pass
        for suffix in ("timer", "service"):
            (self.server.scheduler_unit_dir / f"{unit}.{suffix}").unlink(missing_ok=True)
        self.scheduler_systemctl("daemon-reload")

    def scheduler_next_run(self, schedule):
        if not schedule["enabled"]:
            return None
        unit = scheduler_unit_name(schedule["id"])
        try:
            result = self.scheduler_systemctl("show", f"{unit}.timer", "--property=NextElapseUSecRealtime", "--value")
            return scheduler_next_run_value(result.stdout)
    ...
    def do_POST(self):  # noqa: N802 - stdlib handler API
        ...
        if parts == ["api", "updater-update"]:
            ...
            return
        if parts == ["api", "diagnostics", "interactive-dom"]:
            self.handle_interactive_diagnostic(payload)
            return
        self.send_json(error_payload("UNSUPPORTED_ACTION", "Unsupported API action."), HTTPStatus.METHOD_NOT_ALLOWED)

    def handle_interactive_diagnostic(self, payload):
        """Record one temporary authenticated browser DOM diagnostic on the test node."""
        if not isinstance(payload, dict):
            self.send_json(error_payload("BAD_DIAGNOSTIC", "Diagnostic payload must be an object."), HTTPStatus.BAD_REQUEST)
            return
        allowed = {"kind", "context", "selector", "message", "filename", "line", "column", "stack", "unit", "url"}
        report = {key: payload.get(key, "") for key in allowed}
        for key in ("kind", "context", "selector", "message", "filename", "stack", "unit", "url"):
            if not isinstance(report[key], str):
                report[key] = str(report[key])
            report[key] = report[key][:2000]
        for key in ("line", "column"):
            try:
                report[key] = max(0, min(int(report[key] or 0), 10_000_000))
            except (TypeError, ValueError):
                report[key] = 0
        print("interactive-dom-diagnostic " + json.dumps(report, ensure_ascii=False, sort_keys=True), flush=True)
    ...
    def do_PUT(self):  # noqa: N802
        ...
        if len(parts) == 3 and parts[:2] == ["api", "schedules"]:
            try:
                self.handle_scheduler_update(parts[2], payload)
            except KeyError:
                self.send_json(error_payload("SCHEDULE_NOT_FOUND", "That schedule does not exist."), HTTPStatus.NOT_FOUND)
            except (OSError, RuntimeError, ValueError) as error:
                self.send_json(error_payload("SCHEDULE_NOT_SAVED", str(error) or "Schedule was rejected."), HTTPStatus.UNPROCESSABLE_ENTITY)
...
def parse_args():
    ...
    parser.add_argument("--scheduler-file", type=Path, default=DEFAULT_SCHEDULER_FILE)
    parser.add_argument("--scheduler-unit-dir", type=Path, default=DEFAULT_SCHEDULER_UNIT_DIR)
