...
<body>
  ...
  <script>
    ...
    let targetSelection={schema_version:1,check:{},update:{}},targetSelectionConfirmed={schema_version:1,check:{},update:{}},targetSelectionSaveTimer=null,targetSelectionSaveInFlight=false,targetSelectionSaveQueued=false,targetSelectionRevision=0,targetSelectionEnablePromise=null;
    ...
    function updateTargetSelectionIndicator(){const indicator=document.getElementById('target-selection-indicator');if(indicator)indicator.textContent=`Target selection: ${targetSelection.enabled?'Ultimate Updater':'Proxmox tags'}`}
    function paintTargetSelection(){document.querySelectorAll('.target-selection-state').forEach(button=>{const state=targetSelection[button.dataset.selectionScope]?.[button.dataset.selectionId]||'';button.className=`target-selection-state ${state}`;button.textContent=state==='only'?'✓':state==='exclude'?'✕':'';button.title=selectionLabel(state);button.setAttribute('aria-label',`${button.dataset.selectionScope} selection: ${selectionLabel(state)}`)});updateTargetSelectionIndicator()}
    ...
    async function loadTargetSelection(){clearTimeout(targetSelectionSaveTimer);targetSelectionSaveQueued=false;targetSelectionRevision+=1;targetSelectionEnablePromise=null;try{const data=await api('/api/target-selection');targetSelection={...(data.selection||targetSelection),enabled:data.enabled===true};targetSelectionConfirmed=copyTargetSelection(targetSelection);updateTargetSelectionIndicator();paintTargetSelection()}catch(error){managementMessage('config-message',error.message,true)}}
