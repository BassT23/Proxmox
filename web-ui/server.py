...
<head>
  ...
  <style>
    ...
    * { box-sizing:border-box } body { margin:0; min-height:100vh; color:var(--text); background:radial-gradient(circle at top right,#1e3567 0,var(--bg) 42rem) }
    ...
    .app-shell { display:grid; grid-template-columns:232px minmax(0,1fr); min-height:100vh } .sidebar { display:flex; flex-direction:column; gap:34px; padding:28px 18px; border-right:1px solid var(--line); background:#0b1224aa } .brand { display:flex; align-items:center; gap:10px; padding:0 10px; letter-spacing:-.03em } .brand-mark { display:grid; place-items:center; width:34px; height:34px; border-radius:10px; color:#0b1020; background:var(--accent); font-size:.7rem; font-weight:850 } .brand small { display:block; color:var(--muted); font-size:.72rem; margin-top:1px } nav { display:grid; gap:5px } .nav-item { padding:11px 12px; border-radius:10px; color:var(--muted); text-decoration:none; font-size:.86rem } .nav-item:hover,.nav-item.active { color:var(--text); background:#73a7ff18 } .sidebar-note { margin-top:auto; padding:12px; color:var(--muted); font-size:.72rem; line-height:1.55 } .sidebar-note span { color:#647491 } .app-main { width:min(1320px,100%); padding:34px clamp(18px,4vw,52px) 54px } .app-header { display:flex; justify-content:space-between; gap:20px; align-items:end; margin-bottom:26px } .systems-panel { padding:22px; border:1px solid var(--line); border-radius:18px; background:#151d34aa; box-shadow:0 18px 50px #00000029 } .view-note { color:var(--muted); font-size:.75rem } .node-group,.external-group { margin-top:18px; border:1px solid var(--line); border-radius:14px; overflow:hidden; background:#0e162b99 } .group-header { display:flex; align-items:center; justify-content:space-between; gap:12px; padding:15px 16px; cursor:pointer } .group-header:hover { background:#73a7ff0d } .group-title { display:flex; align-items:center; gap:10px } .group-title strong { font-size:1rem } .group-title small { color:var(--muted); font-size:.72rem } .group-summary { display:flex; align-items:center; gap:12px; color:var(--muted); font-size:.75rem } .chevron { color:var(--accent); font-size:1.1rem; transition:transform .16s } .node-group.open .chevron,.external-group.open .chevron { transform:rotate(90deg) } .group-body { display:none; padding:0 12px 12px } .node-group.open .group-body,.external-group.open .group-body { display:block } .guest-list { display:grid; gap:6px } .target-row { display:grid; grid-template-columns:minmax(150px,1.5fr) .7fr .8fr .8fr .9fr auto; align-items:center; gap:10px; padding:10px 12px; border-top:1px solid #94a3b815; font-size:.78rem } .target-row:hover { background:#73a7ff0b } .target-row .target-name { font-size:.82rem } .target-row .target-id { margin-top:2px } .row-muted { color:var(--muted); font-size:.72rem } .row-actions { display:flex; justify-content:flex-end; gap:6px } .row-actions button { padding:7px 9px; font-size:.72rem } .target-card { min-height:205px; display:flex; flex-direction:column } .target-card .actions { margin-top:auto } .target-card .target-info { margin-top:15px } .external-group { margin-top:20px } .external-group .group-body { padding-top:2px } .job-toggle { display:flex; align-items:center; gap:10px; border:0; padding:0; background:transparent; color:var(--text); font-weight:700; font-size:1rem } .job-count { color:var(--muted); font-size:.74rem; font-weight:500 } .jobs.collapsed .job-list { display:none } .job-list { margin-top:12px } .job-summary { color:var(--muted); font-size:.75rem } .detail-grid { grid-template-columns:repeat(3,1fr) }
    @media (max-width:900px) { .app-shell { grid-template-columns:1fr } .sidebar { position:static; flex-direction:row; align-items:center; gap:18px; padding:12px 16px; border-right:0; border-bottom:1px solid var(--line) } .sidebar nav { display:flex; overflow:auto } .sidebar-note { display:none } .target-row { grid-template-columns:minmax(140px,1.4fr) .8fr .8fr auto; } .target-row .row-os { display:none } }
...
<body><div class="app-shell">
  <aside class="sidebar"><div class="brand"><span class="brand-mark">UU</span><div><strong>Ultimate</strong><small>Updater</small></div></div><nav><a class="nav-item active" href="#dashboard">Dashboard</a><a class="nav-item" href="#systems">Systems</a><a class="nav-item" href="#jobs">Updates / Jobs</a><a class="nav-item" href="#jobs">Logs</a></nav><div class="sidebar-note">Core-owned execution<br><span>Read-only data view + safe actions</span></div></aside>
  <main class="app-main" id="dashboard">
    <header class="app-header"><div><div class="eyebrow">Universal Systems · Preview</div><h1>Dashboard</h1><p class="subtitle">A compact view of your Proxmox nodes, guests, and external systems.</p></div><div class="meta" id="generated">Loading status…</div></header>
  ...
  <script>
    ...
    function nodeGroup(node,guests,host){const group=document.createElement('section');const open=openNodes.has(node);const updateTotal=[host,...guests].filter(Boolean).map(knownUpdates).filter(Number.isInteger).reduce((a,v)=>a+v,0);group.className=`node-group${open?' open':''}`;group.innerHTML=`<div class="group-header"><div class="group-title"><span class="chevron">›</span><div><strong>${esc(node)}</strong><small>Proxmox node · ${guests.length} guest${guests.length===1?'':'s'}</small></div></div><div class="group-summary"><span>${updateTotal} updates</span>${host?statusTone(host):''}<button class="node-details">Details</button></div></div><div class="group-body"><div class="guest-list"></div></div>`;group.querySelector('.group-header').addEventListener('click',e=>{if(e.target.closest('button'))return;if(openNodes.has(node))openNodes.delete(node);else openNodes.add(node);render(currentStatus)});group.querySelector('.node-details').addEventListener('click',e=>{e.stopPropagation();if(host)renderDetails(host)});const list=group.querySelector('.guest-list');guests.forEach(t=>list.appendChild(targetRow(t)));return group}
    function externalGroup(targets){const group=document.createElement('section');group.className=`external-group${openNodes.has('__external__')?' open':''}`;const updates=targets.map(knownUpdates).filter(Number.isInteger).reduce((a,v)=>a+v,0);group.innerHTML=`<div class="group-header"><div class="group-title"><span class="chevron">›</span><div><strong>External systems</strong><small>${targets.length} target${targets.length===1?'':'s'}</small></div></div><div class="group-summary"><span>${updates} updates</span></div></div><div class="group-body"><div class="guest-list"></div></div>`;group.querySelector('.group-header').addEventListener('click',()=>{if(openNodes.has('__external__'))openNodes.delete('__external__');else openNodes.add('__external__');render(currentStatus)});targets.forEach(t=>group.querySelector('.guest-list').appendChild(targetRow(t)));return group}
    function render(data){currentStatus=data;const ts=Array.isArray(data.targets)?data.targets:[],nodes=ts.filter(isProxmoxNode),guests=ts.filter(t=>t.type==='lxc'||t.type==='vm'),external=ts.filter(t=>!isProxmoxNode(t)&&t.type!=='lxc'&&t.type!=='vm');set('total',ts.length);set('online',ts.filter(t=>t.reachable===true).length);set('updates',ts.map(knownUpdates).filter(Number.isInteger).reduce((a,v)=>a+v,0));set('attention',ts.filter(t=>t.check_status!=='ok').length);set('generated',`Schema ${text(data.schema_version)} · generated ${date(data.generated_at)}`);const list=document.getElementById('targets');list.replaceChildren();if(!ts.length){list.innerHTML='<div class="empty">No target status is available yet. Run a check to populate the view.</div>';return}if(nodes.length){const assigned=new Set();nodes.forEach(host=>{const node=nodeLabel(host),members=guests.filter(t=>targetNode(t,nodes)===node);members.forEach(t=>assigned.add(t.id));list.appendChild(nodeGroup(node,members,host))});const unassigned=guests.filter(t=>!assigned.has(t.id));if(unassigned.length)list.appendChild(nodeGroup('Guests without node assignment',unassigned,null))}else if(guests.length){list.appendChild(nodeGroup('Guests',guests,null))}if(external.length)list.appendChild(externalGroup(external))}
