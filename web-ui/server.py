...
<body><main>
  ...
  <script>
    ...
    async function loadJobLog(unit,node){try{const r=await fetch(`/api/jobs/${encodeURIComponent(unit)}/log`),d=await r.json();if(!r.ok)throw new Error(d.error?.message||'Log unavailable');node.hidden=false;node.textContent=d.log||'(no journal output)'}catch(e){notice(e.message,true)}}
    function renderJobs(){const n=document.getElementById('jobs');if(!jobs.length){n.hidden=true;openJobLogId=null;return}if(openJobLogId&&!jobs.some(j=>j.unit===openJobLogId))openJobLogId=null;n.hidden=false;n.innerHTML='<div class="section-title"><h2>Update jobs</h2><span class="hint">Server-side state · safe across browser/device changes</span></div>'+jobs.map(j=>{const open=j.unit===openJobLogId;return `<div class="job"><code>${esc(j.unit)}</code><span>${esc(j.target)}</span><span class="pill ${j.state==='completed'?'good':j.state==='failed'||j.state==='interrupted'?'bad':'warn'}">${esc(j.state)}</span><button data-job="${esc(j.unit)}">${open?'Hide log':'Show log'}</button><div class="log" id="log-${esc(j.unit)}"${open?'':' hidden'}></div></div>`}).join('');n.querySelectorAll('button[data-job]').forEach(b=>b.addEventListener('click',async()=>{const unit=b.dataset.job;const node=document.getElementById(`log-${unit}`);if(openJobLogId===unit){openJobLogId=null;node.hidden=true;b.textContent='Show log';return}openJobLogId=unit;node.hidden=false;b.textContent='Hide log';await loadJobLog(unit,node)}));if(openJobLogId){const node=document.getElementById(`log-${openJobLogId}`);if(node)loadJobLog(openJobLogId,node)}}
