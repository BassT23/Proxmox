...
<body>
  ...
  <script>
    ...
    const terminalStatus=(value,error=false)=>{const node=document.getElementById('interactive-terminal-status');node.textContent=value;node.className=`pill ${error?'bad':value==='Connected'?'good':'warn'}`};
    const interactiveKeybarState=disabled=>{document.querySelectorAll('[data-interactive-key]').forEach(button=>{button.disabled=disabled});if(!disabled){interactiveKeybarVisibility(true);document.getElementById('interactive-terminal-heading').textContent='Live terminal';document.getElementById('interactive-terminal-help').hidden=false}};
    ...
    const terminalMessage=(value,visible=true)=>{const node=document.getElementById('interactive-terminal-message');node.textContent=value;node.hidden=!visible};
