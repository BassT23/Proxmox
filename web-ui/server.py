...
PAGE = PAGE.replace('</body>', '''<script>
    ...
    const syncInteractiveStopButton=()=>{const state=interactiveTerminal,job=state&&jobs.find(item=>item.unit===state.unit),visible=!!state&&state.mode!=='output'&&job?.state==='running';if(!interactiveStopButton)return;interactiveStopButton.hidden=!visible;interactiveStopButton.disabled=!!state?.stopping;interactiveStopButton.textContent=state?.stopping?'Stopping…':'Stop job'};
    interactiveStopButton?.addEventListener('click',()=>{const state=interactiveTerminal,job=state&&jobs.find(item=>item.unit===state.unit);if(!state||state.mode==='output'||job?.state!=='running'||state.stopping)return;interactiveStopModal.hidden=false;interactiveStopModal.classList.add('open');interactiveStopConfirm.focus()});
...
class StatusHandler(BaseHTTPRequestHandler):
    ...
    def handle_job_cancel(self, unit):
        """Request cancellation of one running interactive job.
        ...
        if not job.get("interactive"):
            self.send_json(error_payload("JOB_NOT_CANCELLABLE", "Only running interactive jobs can be stopped here."), HTTPStatus.CONFLICT)
            return
        try:
