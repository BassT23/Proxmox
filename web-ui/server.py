...
<body>
  ...
  <script>
    ...
    const updateSummary=targets=>{const items=targets.filter(Boolean),normal=items.map(knownNormalUpdates),security=items.map(knownSecurityUpdates),knownNormal=normal.filter(Number.isInteger),knownSecurity=security.filter(Number.isInteger),parts=[];parts.push(knownNormal.length?`${knownNormal.reduce((a,v)=>a+v,0)} normal`:'normal unknown');parts.push(knownSecurity.length===items.length?`${knownSecurity.reduce((a,v)=>a+v,0)} security`:'security unknown');return items.length?parts.join(' · '):'Updates unknown'};
  ...
  <script>
    ...
    const aggregateFieldComplete=field=>{const targets=Array.isArray(currentStatus?.targets)?currentStatus.targets:[];return targets.length&&targets.every(t=>Number.isInteger(t?.[field]))?aggregateField(field):null};
    const renderWithSplitSummary=render;
    render=function(data){renderWithSplitSummary(data);const normal=aggregateField('normal_updates'),security=aggregateFieldComplete('security_updates');set('normal-updates',normal===null?'Unknown':normal);set('security-updates',security===null?'Unknown':security)};
    bootstrap();
  </script>
</main></body></html>"""
