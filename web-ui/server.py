...
<body>
  ...
  <script>
    ...
    function testExternalForm(event){event.preventDefault();const form=document.getElementById('target-modal-form'),button=event.currentTarget,payload=externalFormPayload(form);if(payload.id)testTarget(payload.id,payload,button)}
