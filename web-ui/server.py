...
<body>
  ...
  <script>
    ...
    const INPUT_BATCH_DELAY=15,INPUT_CHUNK_BYTES=2048,RESIZE_DEBOUNCE=150;
    ...
    const openInteractiveTerminal=async unit=>{if(typeof Terminal!=='function'||typeof FitAddon==='undefined'||typeof FitAddon.FitAddon!=='function'){notice('The interactive terminal assets are unavailable.',true);return}if(interactiveTerminal?.unit===unit){interactiveTerminal.terminal.focus();return}if(interactiveTerminal)await disposeInteractiveTerminal(true);let attachment_id=interactiveAttachments.get(unit);try{if(!attachment_id){const result=await api(`/api/jobs/${encodeURIComponent(unit)}/attach`,{method:'POST',body:'{}'});attachment_id=result.attachment_id;if(!attachment_id)throw new Error('The interactive attachment is unavailable.');interactiveAttachments.set(unit,attachment_id);interactiveAttached.add(unit)}const panel=document.getElementById('interactive-terminal-panel'),container=document.getElementById('interactive-terminal'),job=jobs.find(item=>item.unit===unit);panel.hidden=false;document.getElementById('interactive-terminal-title').textContent=job?jobTitle(job):unit;document.getElementById('interactive-terminal-download').href=`/api/jobs/${encodeURIComponent(unit)}/download`;terminalMessage('');interactiveKeybarState(false);const terminal=new Terminal({scrollback:2000,convertEol:false});const fitAddon=new FitAddon.FitAddon();terminal.loadAddon(fitAddon);terminal.open(container);interactiveTerminal={unit,attachment_id,terminal,fitAddon,eventSource:null,inputBuffer:[],inputTimer:null,inputClosed:false,resizeObserver:null,resizeTimer:null,viewportHandler:null,resizeInFlight:false,resizeAgain:false,lastSize:null};const state=interactiveTerminal;terminal.attachCustomKeyEventHandler(event=>{if(event.type!=='keydown')return true;if(event.ctrlKey&&event.key.toLowerCase()==='c'){event.preventDefault();disposeInteractiveTerminal(true);return false}if(event.ctrlKey&&(event.key.toLowerCase()==='d'||event.key.toLowerCase()==='z')){event.preventDefault();terminalMessage('This control key is disabled for the interactive job.',true);return false}return true});terminal.onData(queueInteractiveInput);fitAddon.fit();await sendInteractiveResize(true);const refit=()=>{if(interactiveTerminal!==state)return;clearTimeout(state.resizeTimer);state.resizeTimer=setTimeout(()=>{if(interactiveTerminal===state)sendInteractiveResize(false)},RESIZE_DEBOUNCE)};state.viewportHandler=refit;if(typeof ResizeObserver==='function'){state.resizeObserver=new ResizeObserver(refit);state.resizeObserver.observe(container)}window.addEventListener('orientationchange',refit);window.visualViewport?.addEventListener('resize',refit);terminal.focus();const source=new EventSource(`/api/jobs/${encodeURIComponent(unit)}/stream?attachment_id=${encodeURIComponent(attachment_id)}&from=0`);state.eventSource=source;terminalStatus('Connecting…');source.addEventListener('attached',()=>terminalStatus('Connected'));source.addEventListener('output',event=>{try{terminal.write(terminalBytes(event.data))}catch(error){terminalMessage('Terminal output could not be rendered.',true);terminalStatus('Stream error',true)}});source.addEventListener('truncated',()=>terminalMessage('Earlier terminal output is no longer available. Use “Download full log” for the complete journal.',true));source.addEventListener('heartbeat',()=>{});source.addEventListener('closed',()=>{source.close();if(interactiveTerminal===state){state.inputClosed=true;interactiveKeybarState(true);terminalStatus('Job finished')}});source.onerror=()=>{if(source.readyState===EventSource.CLOSED){terminalStatus('Stream unavailable',true);terminalMessage('The live terminal stream ended. The existing job log remains available.',true)}else terminalStatus('Reconnecting…')};decorateInteractiveJobs()}catch(error){await disposeInteractiveTerminal(true);notice(error.message||'The interactive terminal could not be opened.',true)}};
...
PAGE = PAGE.replace('</head>', '<style>.interactive-controls{grid-column:1/-1;display:flex;align-items:center;gap:8px;flex-wrap:wrap;margin-top:8px;padding-top:8px;border-top:1px solid #159cf033}.interactive-status{color:var(--muted);font-size:.72rem}.interactive-controls button{padding:7px 9px;font-size:.72rem}.interactive-terminal-header-actions{display:flex;align-items:center;gap:8px;flex:0 0 auto}.interactive-terminal-keybar{display:none;align-items:center;justify-content:center;gap:6px;flex-wrap:wrap;margin-top:8px}.interactive-terminal-keybar button{min-width:42px;min-height:40px;padding:7px 10px;font-size:.75rem;white-space:nowrap;touch-action:manipulation}.interactive-terminal-help{margin:8px 0 0;color:var(--muted);font-size:.7rem}@media(max-width:720px){html,body{overflow-x:hidden}.interactive-terminal-panel{place-items:stretch;padding:0}.interactive-terminal-dialog{width:100%;height:100vh;height:100dvh;max-height:none;padding:8px;border-radius:0}.interactive-terminal-dialog .section-title{gap:7px;margin-bottom:7px}.interactive-terminal-dialog .section-title h2{display:none}.interactive-terminal-dialog .section-title .hint{font-size:.76rem}.interactive-terminal-header-actions{gap:5px}.interactive-terminal-header-actions .pill{max-width:115px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.interactive-terminal-header-actions button{min-height:40px;padding:8px 9px;white-space:nowrap}.interactive-terminal{padding:3px;border-radius:6px}.interactive-terminal .xterm{font-size:11px;line-height:1.1}.interactive-terminal-keybar{display:flex}.interactive-terminal-actions{margin-top:6px;justify-content:flex-end}.interactive-terminal-actions .job-download{padding:5px 8px;font-size:.68rem}.interactive-terminal-help{margin:5px 0;font-size:.64rem}.interactive-terminal-dialog .management-message{margin:5px 0!important;max-height:44px;overflow:auto;font-size:.68rem}}@media(max-width:380px){.interactive-terminal-dialog{padding:5px}.interactive-terminal-keybar{gap:3px}.interactive-terminal-keybar button{min-width:38px;min-height:40px;padding:6px 7px;font-size:.7rem}.interactive-terminal-header-actions button{padding-inline:7px}.interactive-terminal-header-actions .pill{max-width:82px}.interactive-terminal-help{display:none}}@media(orientation:landscape) and (max-width:720px){.interactive-terminal-keybar{margin-top:4px}.interactive-terminal-help{display:none}}</style></head>', 1)
