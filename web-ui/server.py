...
<body>
  ...
  <script>
    ...
    const scheduleInteractiveResize=()=>{const state=interactiveTerminal;if(!state)return;state.fitAddon.fit();clearTimeout(state.resizeTimer);state.resizeTimer=setTimeout(()=>{if(interactiveTerminal===state)sendInteractiveResize(false)},RESIZE_DEBOUNCE)};
    ...
    const interactiveKeybarState=disabled=>document.querySelectorAll('[data-interactive-key]').forEach(button=>{button.disabled=disabled});
