    ...
    if [[ -f /etc/update-motd.d/01-welcome-screen ]]; then
      ...
      if grep -q "update -check" "$CRON_FILE"; then
        if ! grep -q "RUN_FROM_CRON=true.*update -check" "$CRON_FILE"; then
          ...
          echo "00 06   * * *   root RUN_FROM_CRON=true /usr/local/sbin/update.sh -check >/dev/null 2>&1" >> "$CRON_FILE"
        fi
      fi
    else
...
WELCOME_SCREEN_INSTALL () {
  ...
  if ! grep -q "check-updates.sh" /etc/crontab; then
    echo "00 06   * * *   root RUN_FROM_CRON=true /usr/local/sbin/update.sh -check >/dev/null 2>&1" >> /etc/crontab
  fi
