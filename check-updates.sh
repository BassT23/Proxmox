...
# shellcheck disable=SC2034,SC2086,SC2100,SC2317
  ...
  while [[ $# -gt 0 ]]; do
    ...
    case "$ARGUMENT" in
      ...
      cvm)
        ...
        OUTPUT_TO_FILE
        if [[ -n "${2:-}" ]]; then
          if declare -f CHECK_VM_LIFECYCLE >/dev/null 2>&1; then
            if CHECK_VM_LIFECYCLE "$2"; then check_rc=0; else check_rc=$?; fi
          elif CHECK_VM "$2"; then
            check_rc=0
          else
            check_rc=$?
          fi
...
REMOTE_STATUS_DIAGNOSTICS () {
  ...
  # Retrieval/correlation details are internal diagnostics.  A failed target
  # is reported separately by REMOTE_STATUS_FAILURE_SUMMARY; do not expose
  # transport and artifact metadata in the normal user log.
...
CHECK_HOST () {
  ...
  if [[ -f "$STATUS_MODEL_SCRIPT" ]]; then
    ...
    # Remote workers contribute records to the central summary.  Their local
...
PY
  ...
  elif [[ "${DEBUG:-false}" == true && "$remote_diagnostics_found" != true ]]; then
...
CHECK_HOST_ITSELF () {
  ...
  if [[ "$HOST_UPDATES" -gt 0 || "$REBOOT_REQUIRED" == true ]]; then
...
CHECK_VM_LIFECYCLE () {
  local target="$1" status check_rc=0 lifecycle_failure=0 lifecycle_message=""
  if [[ ! "$target" =~ ^[0-9]+$ ]]; then
    printf 'Cannot check VM: missing or invalid VMID\n' >&2
    return 2
  fi
  SSH_START_DELAY_TIME=$(SANITIZE_NUMBER "${VM_START_DELAY:-45}")
  SSH_START_DELAY_TIME=${SSH_START_DELAY_TIME:-45}
